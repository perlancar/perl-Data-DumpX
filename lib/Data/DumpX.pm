...
sub run_event {
    ...
    my $name = $args{name};
    log_trace "[Data::DumpX] -> run_event(%s)", \%args;
  ...
  RUN_BEFORE_EVENT_HANDLERS:
    {
        ...
        for my $rec (@{ $Handlers{$before_name} }) {
            ...
            log_trace "[Data::DumpX] [event %s] [%d/%d] -> handler %s ...",
                $before_name, $i, scalar(@{ $Handlers{$before_name} }), $label;
            $res = $handler->($Stash);
            $is_success = $res->[0] =~ /\A[123]/;
            log_trace "[Data::DumpX] [event %s] [%d/%d] <- handler %s: %s (%s)",
                $before_name, $i, scalar(@{ $Handlers{$before_name} }), $label,
                $res, $is_success ? "success" : "fail";
            if ($res->[0] == 601) {
                if ($allow_before_handler_to_cancel_event) {
                    log_trace "[Data::DumpX] Cancelling event $name (status 601)";
                    goto RETURN;
                } else {
            ...
            if ($res->[0] == 201) {
                if ($allow_before_handler_to_skip_rest) {
                    log_trace "[Data::DumpX] Skipping the rest of the $before_name handlers (status 201)";
                    last RUN_BEFORE_EVENT_HANDLERS;
                } else {
                    log_trace "[Data::DumpX] $before_name handler returns 201, but we ignore it because allow_before_handler_to_skip_rest is set to false";
                }
            }
        }
    }
  ...
  RUN_EVENT_HANDLERS:
        ...
        for my $rec (@{ $Handlers{$name} }) {
            ...
            if ($res->[0] == 602) {
                if ($allow_handler_to_repeat_event) {
                    log_trace "[Data::DumpX] Repeating event $name (handler returns 602)";
            ...
            if ($res->[0] == 201) {
                if ($allow_handler_to_skip_rest) {
                    log_trace "[Data::DumpX] Skipping the rest of the $name handlers (status 201)";
                    last RUN_EVENT_HANDLERS;
                } else {
                    log_trace "[Data::DumpX] $name handler returns 201, but we ignore it because allow_handler_to_skip_rest is set to false";
                }
            }
    ...
    if ($is_success && $args{on_success}) {
        log_trace "[Data::DumpX] Running on_success ...";
        $args{on_success}->($Stash);
    } elsif (!$is_success && $args{on_failure}) {
        log_trace "[Data::DumpX] Running on_failure ...";
        ...
        for my $rec (@{ $Handlers{$after_name} }) {
            ...
            if ($res->[0] == 602) {
                if ($allow_after_handler_to_repeat_event) {
                    log_trace "[Data::DumpX] Repeating event $name (status 602)";
                    goto RUN_EVENT_HANDLERS;
                } else {
            ...
            if ($res->[0] == 201) {
                if ($allow_after_handler_to_skip_rest) {
                    log_trace "[Data::DumpX] Skipping the rest of the $after_name handlers (status 201)";
                    last RUN_AFTER_EVENT_HANDLERS;
                } else {
                    log_trace "[Data::DumpX] $after_name handler returns 201, but we ignore it because allow_after_handler_to_skip_rest is set to false";
                }
            }
        }
    }
  ...
  RETURN:
    log_trace "[Data::DumpX] <- run_event(name=%s)", $name;
    undef;
}
...
sub activate_plugin {
    ...
    run_event(
        ...
        on_success => sub {
            my $package = "Data::DumpX::$plugin_name";
            (my $package_pm = "$package.pm") =~ s!::!/!g;
            log_trace "[Data::DumpX] Loading module $package ...";
            require $package_pm;
...
sub import {
  ...
  READ_ENV:
    {
        last if $read_env;
      READ_DATA_DUMPX_IMPORT:
        {
            last unless defined $ENV{DATA_DUMPX_IMPORT};
            log_trace "[Data::DumpX] Reading env variable DATA_DUMPX_IMPORT ...";
            _import(_unflatten_import($ENV{DATA_DUMPX_IMPORT}, "DATA_DUMPX_IMPORT"));
            $read_env++;
            last READ_ENV;
        }

      READ_DATA_DUMPX_IMPORT_JSON:
        {
            last unless defined $ENV{DATA_DUMPX_IMPORT_JSON};
            require JSON::PP;
            log_trace "[Data::DumpX] Reading env variable DATA_DUMPX_IMPORT_JSON ...";
            my $imports = JSON::PP::decode_json($ENV{DATA_DUMPX_IMPORT_JSON});
            _import(@$imports);
            $read_env++;
            last READ_ENV;
        }
    ...
    if (@_ && $_[0] =~ /\A-/) {
        # user that specify imports on command-line, e.g. using -MData::DumpX=...
        # can use the ENV syntax so she can specify plugin arguments more
        # easily: -MData::DumpX=-Run,command,foobar,-AnotherPlugin,...
...
1;
# ABSTRACT: A plugin-based data dumper
...
=head1 SYNOPSIS
 ...
 use Data::DumpX;
 Data::DumpX->dump($some_data_structure);
...
registering a handler for it. In other frameworks, events are ssometimes called
hooks.

=head3 Event handler

A coderef that will be called by Data::DumpX on an L<event|/Event>. The event
...
A Perl module under the C<Data::DumpX::> namespace that supplies additional
...
None exported by default, but they are exportable. For most users, knowing just
L</dump> and L</dd> suffices. The rest of the functions are relevant when
wanting to write a plugin.

=head2 dump

Usage:

 my $result = dump($some, \@data, %structure);

Dump data structure and return the result (usually text).

Internally, this is actually just a shortcut for running the C<dump> event:

 { local $Stash->{data} = [$some, \@data, %structure];
   run_event(name => 'dump'); }
...
Usage:
 ...
 dd($some, \@data, \%structure);

Dump data structure and print the result. Also return the original data
structure. Equivalent to:

 sub dd { my $result = dump(@_); print $result; return @_ }
...
Load plugin named C<$name> (by loading Perl module C<Data::DumpX::$name>),
instantiate it with arguments %$args, then call the object method C<activate()>.

Note: there is a special plugin C<DisablePlugin|Data::DumpX::DisablePlugin> which
...
=head2 DATA_DUMPX_IMPORT

String. Additional import, will be added at the first C<import()> before the
usual import arguments. Used to add plugins for a running script, e.g. to add
...
For example, this:
 ...
 use Data::DumpX
     'ObjectAsString',
     Partial => {max_hash_keys=>10, max_array_elems=>10};

should be written as:

 DATA::DUMPX_IMPORT=-ObjectAsString,-Partial,max_hash_keys,10,max_array_elems,10

If your script is:

 use Data::DumpX Color => {theme=>'Foo'};
...
C<@EVENT> or C<@EVENT@PRIO>. For example: XXX.

=head2 DATA_DUMPX_IMPORT_JSON

String (JSON-encoded array). This is an alternative to L</DATA_DUMPX_IMPORT> and
has a lower precedence (will not be evaluated when C<DATA_DUMPX_IMPORT> is
defined). Useful if a plugin accept data structure instead of plain scalars.

Example:

 DATA_DUMPX_IMPORT_JSON='["ObjectAsString","Partial",{"max_hash_keys":10,"max_array_elems":10},"Color",{"theme":"Foo"}]


=head1 SEE ALSO

The various plugins under the C<Data::DumpX::> namespace.

Other dumpers, among others: L<Data::Dumper>, L<Data::Dump>,
L<Data::Dump::Color>, L<Data::Dmp>, L<Data::Printer>, L<XXX>.
